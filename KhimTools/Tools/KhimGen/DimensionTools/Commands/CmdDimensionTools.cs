...
namespace KhimTools.DimensionTools.Commands
{
    ...
    public sealed class CmdDimensionTools : IExternalCommand
    {
        ...
        private static DimensionResult CreateSpot(Document doc, DimensionPlan plan)
        {
            ...
            DimensionReferenceInfo info = plan.Context.References[0]; Reference resolved = DimensionReferenceService.ResolveStableReference(doc, info); if (resolved == null) return new DimensionResult { Operation = DimensionOperation.SPOT_ELEVATION, Status = DimensionStatus.STALE_REFERENCE, Message = "Spot face Reference is stale." }; string live = DimensionReferenceService.ComputeLiveGeometryFingerprint(doc, info, resolved); if (!string.Equals(live, info.GeometryFingerprint, StringComparison.Ordinal)) return new DimensionResult { Operation = DimensionOperation.SPOT_ELEVATION, Status = DimensionStatus.STALE_DIMENSION_PLAN, Message = "Spot source geometry changed after Preview." }; DimensionReferenceService.RefreshResolvedGeometry(doc, plan.Context.View, info, resolved); ViewPlane plane = ViewPlane.FromView(plan.Context.View); XYZ target = info.WorldPoint ?? XYZ.Zero; double offset = DimensionGeometryService.Mm(Math.Abs(plan.Context.Options.OffsetMillimeters) < 0.1 ? 100 : plan.Context.Options.OffsetMillimeters); XYZ origin = target + plane.RightDirection * offset; XYZ bend = origin + plane.UpDirection * DimensionGeometryService.Mm(20); XYZ end = bend + plane.RightDirection * DimensionGeometryService.Mm(20); return ElevationDimensionService.CreateSpotElevation(doc, plan.Context.View, resolved, origin, bend, end, target, ElementId.InvalidElementId);
        }
    }
}
