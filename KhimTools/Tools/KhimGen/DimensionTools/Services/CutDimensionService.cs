...
namespace KhimTools.DimensionTools.Services
{
    public static class CutDimensionService
    {
        ...
        {
            Dimension original = doc == null ? null : doc.GetElement(dimensionId) as Dimension; var result = new DimensionResult { Operation = DimensionOperation.CUT }; if (original == null) { result.Status = DimensionStatus.INVALID_SELECTION; result.Message = "Dimension is unavailable."; return result; } if (original.Pinned) { result.Status = DimensionStatus.PINNED; result.Message = "Pinned dimension is blocked by default."; return result; }
