...
namespace KhimTools.DimensionTools.Services
{
    public static class ElevationDimensionService
    {
        ...
        public static DimensionResult CreateSpotElevation(Document doc, View view, Reference reference, XYZ origin, XYZ bend, XYZ end, XYZ refPoint, ElementId typeId)
        {
            ...
            using (var group = new TransactionGroup(doc, "K-TOOLS Spot Elevation")) { group.Start(); try { using (var tx = new Transaction(doc, "Create Spot Elevation")) { tx.Start(); SpotDimension spot = new DimensionApiAdapter().CreateSpotElevation(doc, view, reference, origin, bend, end, refPoint); if (spot == null) { tx.RollBack(); result.Status = DimensionStatus.FAILED; result.Message = "NewSpotElevation returned null."; group.RollBack(); return result; } if (typeId != null && typeId != ElementId.InvalidElementId) spot.ChangeTypeId(typeId); doc.Regenerate(); if (doc.GetElement(spot.Id) == null || spot.OwnerViewId != view.Id) { tx.RollBack(); group.RollBack(); result.Status = DimensionStatus.POST_VERIFY_FAILED; result.Message = "Spot Elevation post verification failed."; return result; } tx.Commit(); result.CreatedDimensionIds.Add(spot.Id); result.Status = DimensionStatus.CREATED; result.VerificationPassed = true; result.Message = "Spot Elevation created and verified."; } group.Assimilate(); } catch (System.Exception ex) { if (group.GetStatus() == TransactionStatus.Started) group.RollBack(); result.Status = DimensionStatus.FAILED; result.Message = ex.Message; } } return result;
        }
    }
}
