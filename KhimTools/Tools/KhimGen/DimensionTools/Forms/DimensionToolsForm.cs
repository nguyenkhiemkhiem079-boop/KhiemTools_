...
namespace KhimTools.DimensionTools.Forms
{
    ...
    {
        private readonly Document _doc; private readonly RevitView _view; private readonly IList<ElementId> _selection; private readonly ComboBox _operation = new ComboBox(); private readonly ComboBox _axis = new ComboBox(); private readonly ComboBox _strategy = new ComboBox(); private readonly NumericUpDown _offset = new NumericUpDown(); private readonly NumericUpDown _boundary = new NumericUpDown(); private readonly Label _status = new Label(); private Button _apply; public DimensionPlan Plan { get; private set; }
        public DimensionToolsForm(Document doc, RevitView view, IList<ElementId> selection) { _doc = doc; _view = view; _selection = selection ?? new List<ElementId>(); KhimUiStyle.ApplyFormTheme(this); BuildLayout(); }
        ...
        private DimensionPlan BuildPlan(DimensionOperation operation, DimensionOptions options)
        {
            switch (operation)
            {
                ...
                case DimensionOperation.JOIN:
                case DimensionOperation.REMOVE_ZERO:
                case DimensionOperation.MOVE_TEXT: return BuildEditPlan(operation, options);
        ...
        {
            var context = new DimensionContext { Document = _doc, View = _view, Operation = operation, Options = options ?? new DimensionOptions() };
            foreach (ElementId id in _selection ?? new List<ElementId>()) if (_doc.GetElement(id) is Dimension) context.DimensionIds.Add(id);
