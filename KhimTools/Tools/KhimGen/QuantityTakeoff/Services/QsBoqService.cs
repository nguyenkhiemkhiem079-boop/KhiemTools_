...
namespace KhimTools.QuantityTakeoff.Services
{
    public static class QsBoqService
    {
        ...
        {
            if (item == null) return new QsClassificationResult { Status = QsClassificationStatus.Unclassified, Message = "Missing quantity line" };
            var matches = (rules ?? Enumerable.Empty<QsBoqRule>()).Where(r => r != null && r.Enabled && (activeCodes == null || activeCodes.Contains(r.TargetBoqCode)) && Match(r, item)).OrderBy(r => r.Priority).ThenByDescending(Specificity).ThenBy(r => r.Id, StringComparer.Ordinal).ToList();
        ...
        private static bool Match(QsBoqRule r, QtoLine x) => r != null && MatchText(r.Category, x.Category) && MatchText(r.FamilyPattern, x.FamilyName) && MatchText(r.TypePattern, x.TypeName) && MatchText(r.MaterialPattern, x.Material) && MatchText(r.MeasurementCode, x.Code);
