...
namespace KhimTools.ModifyObjects.General
{
    public static class Array3DService
    {
        ...
        public static ModifyObjectResult Execute(Document doc, ModifyObjectPlan plan) { return ModifyObjectExecutionService.Execute(doc, plan, () => { var result = new ModifyObjectResult(); using (var tx = new Transaction(doc, "K-TOOLS Array 3D")) { KhimTools.Core.Revit.TransactionBoundary.Start(tx, "ModifyObjects.Array3DService"); XYZ vector = plan.Context.ArrayVector.ToXyz(); for (int index = 1; index < plan.Context.ArrayCount; index++) { ICollection<ElementId> copies = ElementTransformUtils.CopyElement(doc, plan.Context.PrimaryElementId, new XYZ(vector.X * index, vector.Y * index, vector.Z * index)); foreach (ElementId copy in copies) result.CreatedElementIds.Add(copy); } doc.Regenerate(); KhimTools.Core.Revit.TransactionBoundary.Commit(tx, "ModifyObjects.Array3DService"); } if (plan.Context.IncludeOriginal) result.ModifiedElementIds.Add(plan.Context.PrimaryElementId); result.Status = ModifyObjectStatus.CREATED; result.Summary = "Deterministic 3D array created; include original = " + plan.Context.IncludeOriginal; return result; }); }
    }
}
