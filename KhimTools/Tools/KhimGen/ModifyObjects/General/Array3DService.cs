...
namespace KhimTools.ModifyObjects.General
{
    public static class Array3DService
    {
        ...
        public static ModifyObjectResult Execute(ModifyObjectPlan plan) { return ModifyObjectExecutionService.Execute(plan, () => { Document doc = plan.Context.Document; var result = new ModifyObjectResult(); using (var tx = new Transaction(doc, "K-TOOLS Array 3D")) { tx.Start(); for (int index = 1; index < plan.Context.ArrayCount; index++) { ElementId copy = ElementTransformUtils.CopyElement(doc, plan.Context.PrimaryElementId, new XYZ(plan.Context.ArrayVector.X * index, plan.Context.ArrayVector.Y * index, plan.Context.ArrayVector.Z * index)); result.CreatedElementIds.Add(copy); } doc.Regenerate(); tx.Commit(); } if (plan.Context.IncludeOriginal) result.ModifiedElementIds.Add(plan.Context.PrimaryElementId); result.Status = ModifyObjectStatus.CREATED; result.Summary = "Deterministic 3D array created; include original = " + plan.Context.IncludeOriginal; return result; }); }
    }
}
