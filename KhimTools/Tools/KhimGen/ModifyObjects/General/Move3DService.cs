...
namespace KhimTools.ModifyObjects.General
{
    public static class Move3DService
    {
        ...
        public static ModifyObjectResult Execute(ModifyObjectPlan plan) { return ModifyObjectExecutionService.Execute(plan, () => { var result = new ModifyObjectResult(); using (var tx = new Transaction(plan.Context.Document, "K-TOOLS Move 3D")) { tx.Start(); ElementTransformUtils.MoveElements(plan.Context.Document, plan.Context.ElementIds, plan.Context.MoveVector); plan.Context.Document.Regenerate(); tx.Commit(); } foreach (ElementId id in plan.Context.ElementIds) result.ModifiedElementIds.Add(id); result.Status = ModifyObjectStatus.MODIFIED; result.Summary = "Selected elements moved by a unit-aware 3D vector."; return result; }); }
    }
}
