...
namespace KhimTools.ModifyObjects.General
{
    public static class Move3DService
    {
        ...
        public static ModifyObjectResult Execute(Document doc, ModifyObjectPlan plan) { return ModifyObjectExecutionService.Execute(doc, plan, () => { var result = new ModifyObjectResult(); using (var tx = new Transaction(doc, "K-TOOLS Move 3D")) { tx.Start(); ElementTransformUtils.MoveElements(doc, plan.Context.ElementIds, plan.Context.MoveVector.ToXyz()); doc.Regenerate(); tx.Commit(); } foreach (ElementId id in plan.Context.ElementIds) result.ModifiedElementIds.Add(id); result.Status = ModifyObjectStatus.MODIFIED; result.Summary = "Selected elements moved by a unit-aware 3D vector."; return result; }); }
    }
}
