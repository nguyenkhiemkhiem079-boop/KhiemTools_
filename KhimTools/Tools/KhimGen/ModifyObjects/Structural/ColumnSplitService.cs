...
namespace KhimTools.ModifyObjects.Structural
{
    public static class ColumnSplitService
    {
        ...
        private static ModifyObjectResult ExecuteIsolated(Document doc, ModifyObjectPlan plan)
        {
            ...
            var created = new System.Collections.Generic.List<FamilyInstance>(); using (var tx = new Transaction(doc, "K-TOOLS Split Column")) { tx.Start(); FamilySymbol symbol = doc.GetElement(source.GetTypeId()) as FamilySymbol; LocationPoint point = source.Location as LocationPoint; FamilyInstance lower = doc.Create.NewFamilyInstance(point.Point, symbol, baseLevel, StructuralType.Column); FamilyInstance upper = doc.Create.NewFamilyInstance(point.Point, symbol, splitLevel, StructuralType.Column); SetTop(lower, splitLevel.Id); SetTop(upper, topLevel.Id); CopyParameters(source, lower, result); CopyParameters(source, upper, result); ModifyObjectElementHelpers.PreservePointRotation(doc, source, lower); ModifyObjectElementHelpers.PreservePointRotation(doc, source, upper); doc.Regenerate(); if (lower == null || upper == null) { tx.RollBack(); return new ModifyObjectResult { Status = ModifyObjectStatus.FAILED, Message = "Revit did not create both replacement columns." }; } doc.Delete(source.Id); tx.Commit(); created.Add(lower); created.Add(upper); }
