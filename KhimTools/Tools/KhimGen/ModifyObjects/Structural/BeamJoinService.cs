...
namespace KhimTools.ModifyObjects.Structural
{
    public static class BeamJoinService
    {
        ...
        private static ModifyObjectResult ExecuteIsolated(Document doc, ModifyObjectPlan plan)
        {
            ...
            using (var tx = new Transaction(doc, "K-TOOLS Join Beams")) { KhimTools.Core.Revit.TransactionBoundary.Start(tx, "ModifyObjects.BeamJoinService"); a.Curve = Line.CreateBound(start, end); doc.Regenerate(); doc.Delete(second.Id); KhimTools.Core.Revit.TransactionBoundary.Commit(tx, "ModifyObjects.BeamJoinService"); }
