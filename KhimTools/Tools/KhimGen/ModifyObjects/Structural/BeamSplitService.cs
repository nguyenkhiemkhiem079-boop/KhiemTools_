...
namespace KhimTools.ModifyObjects.Structural
{
    public static class BeamSplitService
    {
        ...
        {
            FamilyInstance source = doc.GetElement(plan.Context.PrimaryElementId) as FamilyInstance; LocationCurve location = source == null ? null : source.Location as LocationCurve; if (source == null || location == null || !(location.Curve is Line)) return new ModifyObjectResult { Status = ModifyObjectStatus.BEAM_GEOMETRY_UNSUPPORTED };
            Line line = location.Curve as Line; XYZ split = plan.Context.SplitPoint == null ? null : plan.Context.SplitPoint.ToXyz(); if (split == null) split = line.Evaluate(0.5, true); if (split.DistanceTo(line.GetEndPoint(0)) < ModifyObjectElementHelpers.Millimeters(1) || split.DistanceTo(line.GetEndPoint(1)) < ModifyObjectElementHelpers.Millimeters(1)) return new ModifyObjectResult { Status = ModifyObjectStatus.INVALID_SPLIT_POINT };
            Level level = doc.GetElement(source.LevelId) as Level; FamilySymbol symbol = doc.GetElement(source.GetTypeId()) as FamilySymbol; var result = new ModifyObjectResult(); using (var tx = new Transaction(doc, "K-TOOLS Split Beam")) { KhimTools.Core.Revit.TransactionBoundary.Start(tx, "ModifyObjects.BeamSplitService"); FamilyInstance first = doc.Create.NewFamilyInstance(Line.CreateBound(line.GetEndPoint(0), split), symbol, level, StructuralType.Beam); FamilyInstance second = doc.Create.NewFamilyInstance(Line.CreateBound(split, line.GetEndPoint(1)), symbol, level, StructuralType.Beam); string conflict; ModifyObjectElementHelpers.TryCopySafeParameters(source, first, out conflict); ModifyObjectElementHelpers.TryCopySafeParameters(source, second, out conflict); doc.Regenerate(); if (first == null || second == null) { KhimTools.Core.Revit.TransactionBoundary.RollBack(tx, "ModifyObjects.BeamSplitService"); return new ModifyObjectResult { Status = ModifyObjectStatus.FAILED }; } doc.Delete(source.Id); KhimTools.Core.Revit.TransactionBoundary.Commit(tx, "ModifyObjects.BeamSplitService"); result.CreatedElementIds.Add(first.Id); result.CreatedElementIds.Add(second.Id); }
