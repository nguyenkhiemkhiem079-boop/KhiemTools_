...
namespace KhimTools.ModifyObjects.Structural
{
    public static class ColumnJoinService
    {
        ...
        private static ModifyObjectResult ExecuteIsolated(Document doc, ModifyObjectPlan plan)
        {
            ...
            var result = new ModifyObjectResult { Status = ModifyObjectStatus.DELETED_SOURCE, Summary = "Column pair joined with explicit conflict policy." };
            result.ModifiedElementIds.Add(survivor.Id);
            result.DeletedSourceIds.Add(removed.Id);
            return result;
        }
    }
}
