...
namespace KhimTools.ModifyObjects.Wall
{
    public static class WallSplitService
    {
        ...
        private static ModifyObjectResult ExecuteIsolated(Document doc, ModifyObjectPlan plan)
        {
            ...
            XYZ split = plan.Context.SplitPoint == null ? line.Evaluate(0.5, true) : plan.Context.SplitPoint.ToXyz(); double height = source.get_Parameter(BuiltInParameter.WALL_USER_HEIGHT_PARAM) == null ? ModifyObjectElementHelpers.Millimeters(3000) : source.get_Parameter(BuiltInParameter.WALL_USER_HEIGHT_PARAM).AsDouble(); double offset = source.get_Parameter(BuiltInParameter.WALL_BASE_OFFSET) == null ? 0 : source.get_Parameter(BuiltInParameter.WALL_BASE_OFFSET).AsDouble(); var result = new ModifyObjectResult(); using (var tx = new Transaction(doc, "K-TOOLS Split Wall")) { KhimTools.Core.Revit.TransactionBoundary.Start(tx, "ModifyObjects.WallSplitService"); RevitWall first = RevitWall.Create(doc, Line.CreateBound(line.GetEndPoint(0), split), source.WallType.Id, level.Id, height, offset, source.Flipped, false); RevitWall second = RevitWall.Create(doc, Line.CreateBound(split, line.GetEndPoint(1)), source.WallType.Id, level.Id, height, offset, source.Flipped, false); doc.Regenerate(); if (first == null || second == null) { KhimTools.Core.Revit.TransactionBoundary.RollBack(tx, "ModifyObjects.WallSplitService"); return new ModifyObjectResult { Status = ModifyObjectStatus.FAILED, Message = "Wall replacement creation failed." }; } string conflict; ModifyObjectElementHelpers.TryCopySafeParameters(source, first, out conflict); ModifyObjectElementHelpers.TryCopySafeParameters(source, second, out conflict); doc.Delete(source.Id); KhimTools.Core.Revit.TransactionBoundary.Commit(tx, "ModifyObjects.WallSplitService"); result.CreatedElementIds.Add(first.Id); result.CreatedElementIds.Add(second.Id); }
