...
namespace KhimTools.ModifyObjects.Wall
{
    public static class WallSplitService
    {
        ...
        {
            Document doc = plan.Context.Document; Wall source = doc.GetElement(plan.Context.PrimaryElementId) as Wall; LocationCurve location = source == null ? null : source.Location as LocationCurve; Line line = location == null ? null : location.Curve as Line; Level level = source == null ? null : doc.GetElement(source.LevelId) as Level; if (line == null || level == null) return new ModifyObjectResult { Status = ModifyObjectStatus.INVALID_GEOMETRY };
            XYZ split = plan.Context.SplitPoint == null ? line.Evaluate(0.5, true) : plan.Context.SplitPoint; double height = source.get_Parameter(BuiltInParameter.WALL_USER_HEIGHT_PARAM) == null ? ModifyObjectElementHelpers.Millimeters(3000) : source.get_Parameter(BuiltInParameter.WALL_USER_HEIGHT_PARAM).AsDouble(); double offset = source.get_Parameter(BuiltInParameter.WALL_BASE_OFFSET) == null ? 0 : source.get_Parameter(BuiltInParameter.WALL_BASE_OFFSET).AsDouble(); var result = new ModifyObjectResult(); using (var tx = new Transaction(doc, "K-TOOLS Split Wall")) { tx.Start(); Wall first = Wall.Create(doc, Line.CreateBound(line.GetEndPoint(0), split), source.WallType.Id, level.Id, height, offset, source.Flipped, false); Wall second = Wall.Create(doc, Line.CreateBound(split, line.GetEndPoint(1)), source.WallType.Id, level.Id, height, offset, source.Flipped, false); doc.Regenerate(); if (first == null || second == null) { tx.RollBack(); return new ModifyObjectResult { Status = ModifyObjectStatus.FAILED, Message = "Wall replacement creation failed." }; } string conflict; ModifyObjectElementHelpers.TryCopySafeParameters(source, first, out conflict); ModifyObjectElementHelpers.TryCopySafeParameters(source, second, out conflict); doc.Delete(source.Id); tx.Commit(); result.CreatedElementIds.Add(first.Id); result.CreatedElementIds.Add(second.Id); }
