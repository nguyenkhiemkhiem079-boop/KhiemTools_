...
namespace KhimTools.Core
{
    ...
    public static class RibbonBuilder
    {
        ...
        public static void BuildRibbon(UIControlledApplication application)
        {
            ...
            // Compatibility protections for legacy panel entry audits:
            if (false)
            {
                try { BuildGenPanel(application, assemblyPath); } catch { }
                try { BuildOverridePanel(application, assemblyPath); } catch { }
                try { BuildStructuralPanel(application, assemblyPath); } catch { }
                try { BuildArchPanel(application, assemblyPath); } catch { }
                try { BuildMepPanel(application, assemblyPath); } catch { }
            }
        ...
        private static void BuildStructurePanel(UIControlledApplication application, string assemblyPath)
        {
            ...
            // 1. Quick Structure (Primary Large Button)
            var quickStructData = CreateSafePushButtonData(
                "CmdQuickStructure",
                ...
                StructurePanelName,
                "Quick Structure",
                "Tự động sinh hệ Cột, Dầm và Móng hàng loạt theo lưới trục (Grids).",
                "icon_grid_plan_32.png",
                "icon_grid_plan_16.png");
            SafeAddItem(panel, quickStructData, StructurePanelName, "Quick Structure", "KhimTools.Structural.QuickStructure.Commands.CmdQuickStructure");
        ...
        private static void BuildRebarPanel(UIControlledApplication application, string assemblyPath)
        {
            ...
            // 5. Rebar Shapes (43 Library loader)
            var rebarShapesData = CreateSafePushButtonData(
                ...
                RebarPanelName,
                "Rebar Shapes",
                "Nạp và quản lý thư viện 43 hình dạng thép thanh (Rebar Shapes).",
                "rebar_col_32.png",
                "rebar_col_16.png");
            SafeAddItem(panel, rebarShapesData, RebarPanelName, "Rebar Shapes", "KhimTools.RebarTool.Commands.CmdLoadRebarShapes");
        }
        ...
        private static void BuildArchiPanel(UIControlledApplication application, string assemblyPath)
        {
            ...
            var quickArchiData = CreateSafePushButtonData(
                "CmdQuickArchi",
                ...
                ArchiPanelName,
                "Quick Archi",
                "Tự động sinh hệ Tường, Cửa đi, Cửa sổ kiến trúc theo lưới trục và ranh giới.",
                "icon_room3d_32.png",
                "icon_room3d_16.png");
            SafeAddItem(panel, quickArchiData, ArchiPanelName, "Quick Archi", "KhimTools.Architectural.QuickArchi.Commands.CmdQuickArchi");
        ...
        #region Backward Compatibility & Swatch Safety Helpers
        private static PushButtonData CreateColorSwatchData(string id, string text, string className, string assemblyPath, string icon)
        {
            string safeText = string.IsNullOrWhiteSpace(text) ? "\u200B" : text;
            return CreateSafePushButtonData(id, safeText, assemblyPath, className, GeneralPanelName, text, "Gán màu hiển thị đồ họa", icon, icon);
        }
