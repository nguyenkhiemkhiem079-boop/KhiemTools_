...
| TD-012 | Rebar golden regression | P1 | Phase 17 now has actual production anchorage/lap calculator goldens and input-boundary tests, but no semantic golden comparison against Rebar generator geometry, and generator-level geometry edge cases remain unaccepted. Comparator checks still use synthetic DTO values; Rebar host scenarios are registered but unexecuted. | No isolated disposable Revit fixture model or supported host runner was available; expected geometry must not be fabricated. | Phase 17 exit gate | Medium to high; production geometry and generator edge behavior lack golden/edge regression evidence. |
...
`TECH_DEBT_P0 = 0`, `TECH_DEBT_P1 = 8`, `TECH_DEBT_P2 = 3`, `TECH_DEBT_P3 = 0`.
