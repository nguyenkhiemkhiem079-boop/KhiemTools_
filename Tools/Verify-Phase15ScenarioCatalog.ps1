...
Assert ($catalog.manualScenarios.id -contains 'COLUMN_CREATE_UNDO' -and $catalog.manualScenarios.id -contains 'COLUMN_GOLDEN_LAYOUT' -and $catalog.manualScenarios.id -contains 'CIRCULAR_COLUMN_CREATE_UNDO' -and $catalog.manualScenarios.id -contains 'CIRCULAR_COLUMN_CANCEL' -and $catalog.manualScenarios.id -contains 'CIRCULAR_COLUMN_DUPLICATE' -and $catalog.manualScenarios.id -contains 'BEAM_CANCEL' -and $catalog.manualScenarios.id -contains 'SLAB_DUPLICATE') 'Rectangular/circular column, beam, and slab golden/create/cancel/undo/duplicate scenarios are covered'
...
Assert ($catalog.manualScenarios.Count -eq 15) 'All fifteen mandatory Rebar manual scenarios are registered'
$requiredRebarScenarios = @('COLUMN_CREATE_UNDO', 'COLUMN_GOLDEN_LAYOUT', 'COLUMN_CANCEL', 'COLUMN_DUPLICATE', 'CIRCULAR_COLUMN_CREATE_UNDO', 'CIRCULAR_COLUMN_CANCEL', 'CIRCULAR_COLUMN_DUPLICATE', 'BEAM_CREATE_UNDO', 'BEAM_CANCEL', 'BEAM_DUPLICATE', 'SLAB_CREATE_UNDO', 'SLAB_CANCEL', 'SLAB_DUPLICATE')
