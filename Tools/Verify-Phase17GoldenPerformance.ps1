...
$text = $output -join "`n"
if ($text -notmatch 'GOLDEN_REGRESSION_ACCEPTANCE=PASS \(48 assertions\)' -or
    ...
    throw 'Phase 17 harness did not report the expected 48 executed assertions and production Rebar-domain gates.'
}
if ($text -notmatch 'EDGE_CASE_ACCEPTANCE=PASS \(QS/MEP/Architectural domain calculations\)' -or
    ...
    $text -notmatch 'REBAR_HOST_GEOMETRY_GOLDEN=HOST_REQUIRED / NOT_EXECUTED') {
