...
Add-Check 'column-beam-slab-foundation-undo-scenarios-registered' (@('COLUMN_UNDO', 'BEAM_UNDO', 'SLAB_UNDO', 'FOUNDATION_UNDO', 'FOUNDATION_CANCEL', 'FOUNDATION_DUPLICATE' | Where-Object { -not $hostScenarios.Contains($_) }).Count -eq 0) 'Tools\RebarPreviewHostScenarios.md registers host-only create/Undo/cancel/duplicate scenarios as NOT_EXECUTED.'
