...
Add-Check 'slab-profile-fails-closed-on-unsupported-host-geometry' ($slabGeometry.Contains('Slab reinforcement currently supports horizontal planar floors only') -and $slabGeometry.Contains('IsSupportedHorizontalLinearLoop') -and $slabGeometry.Contains('never synthesize a rectangular opening from its bounding box') -and -not $slabGeometry.Contains('ElementIntersectsElementFilter(floor)') -and $slabHostScenarios.Contains('SLAB_UNSUPPORTED_HOST_GEOMETRY')) 'Slab profile analysis rejects sloped/curved geometry and consumes actual top-face loops rather than scanning/synthesizing opening bounding boxes; host case remains unexecuted.'
